export const logo = require('./logo.svg');
export const arrowUp = require('./arrowUp.svg');
export const arrowDown = require('./arrowDown.svg');
export const bitcoin = require('./bitcoin.svg');
export const iota = require('./iota.svg');
export const mnr = require('./mnr.svg');
export const nem = require('./nem.svg');
export const xrp = require('./xrp.svg');
export const byc = require('./byc.svg');
export const moon = require('./moon.png');
export const sun = require('./sun.png');
export const heroBackground = require('./heroBackground.png');
export const crypto_banner = require('./crypto_banner.png');
export const octagon = require('./octagon.png');
export const octagonWhite = require('./octagonWhite.png');
export const arrowUpOutline = require('./arrowUpOutline.svg');
export const arrowUpOutlineWhite = require('./arrowUpOutlineWhite.svg');
export const backgroundFooter = require('./backgroundFooter.png');
export const backgroundFooterWhite = require('./backgroundFooterWhite.png');
export const facebook = require('./facebook.png');
export const twitter = require('./twitter.png');
export const instagram = require('./instagram.png');
export const linkedin = require('./linkedin.png');
export const globe = require('./globe.png');
export const leftOctagons = require('./leftOctagons.png');
export const rightOctagons = require('./rightOctagons.png');
export const person = require('./person.png');
export const leftArrow = require('./leftArrow.png');
export const rightArrow = require('./rightArrow.png');
export const leftArrowWhite = require('./leftArrowWhite.png');
export const rightArrowWhite = require('./rightArrowWhite.png');
export const octagonOutline = require('./octagonOutline.png');
export const box = require('./box.png');
export const coins = require('./coins.png');
export const heart = require('./heart.png');
export const group = require('./group.png');
export const videoImage = require('./videoImage.png');
export const playButton = require('./playButton.png');
export const promotions = require('./promotions.png');
